...
class enquiry_class
{
    ...
    function __construct()
    {
        ...
        $colour = $this->prefs['pref_captcha_colour'];
        if (strlen($colour) > 0)
        {
            define('e_CAPTCHA_FONTCOLOR', $colour); }
        //include_once (e_HANDLER . 'secure_img_handler.php');
        //$sec_img = new secure_image;


        $this->shortcodes->prefs = $this->prefs; $this->guest = check_class('252'); $this->shortcodes->guest = $this->guest; $this->image = ($this->prefs['pref_use_captcha'] == 1 && (extension_loaded("gd") ? true : false));
            $this->shortcodes->use_imagecode = $this->image; if ($this->image && isset($_POST['rand_num']))
        {
    ...
    public function runPage()
    {
        ...
        $class = $this->prefs['viewClass']; if (e107::getUser()->checkClass($class, false))
        {
            $this->text = $this->notPermitted(); //$this->render = true;
            }
    else
    {
        ...
        case 'form' : $this->text = $this->doForm(); break; case 'save' : if ($this->doSave())
            {
                $this->text = $this->enquirySaved(); }
    else
    {
        $this->text = $this->enquiryNotSaved(); }
    break; default : case 'show' : $this->text = $this->enquiryFront(); break; }
}
return $this->text; }
...
function doForm()
{
$this->shortcodes->post = $_POST; $this->text = $this->tp->parseTemplate($this->template->enquiryForm(), true, $this->shortcodes); return $this->text; }
...
$row['enquiry_title'] = intval($_POST['enquiry_title']); $row['enquiry_name'] = $this->tp->toDB($_POST['enquiry_name']); $row['enquiry_address1'] = $this->tp->toDB($_POST['enquiry_address1']); $row['enquiry_address2'] =
$this->tp->toDB($_POST['enquiry_address2']); $row['enquiry_town'] = $this->tp->toDB($_POST['enquiry_town']); $row['enquiry_county'] = $this->tp->toDB($_POST['enquiry_county']); $row['enquiry_phone'] =
$this->tp->toDB($_POST['enquiry_phone']); $row['enquiry_email'] = $this->tp->toDB($_POST['enquiry_email']); $row['enquiry_category'] = (int)($_POST['enquiry_category']); $row['enquiry_otherinfo'] =
$this->tp->toDB($_POST['enquiry_otherinfo']); $row['enquiry_agerange'] = intval($_POST['enquiry_agerange']); $row['enquiry_gender'] = intval($_POST['enquiry_gender']); $row['enquiry_dateposted'] =
time(); if (!$this->image || ($this->image && isset($_POST['rand_num']) && $this->verified_code))
{
$sql = 'INSERT INTO #enquiry_forms ('; $values = 'VALUES ('; foreach ($row as $key => $value)
{
    $sql .= "{$key},"; $values .= "'{$value}',"; }
$sql = substr($sql, 0, -1); $values = substr($values, 0, -1); $values .= ') '; $sql .= ') ' . $values; $this->cache->clear('nomd5_enquiry', false, true); if (e107::getDB()->gen($sql, false) !== false)
{
    e107::getMessage()->addSuccess('Saved'); $id = e107::getDB()->lastInsertId(); $sql = 'SELECT * FROM #enquiry_forms 
            left join #enquiry_categories on enquiry_category = enquiry_category_id
            WHERE enquiry_id = ' . $id . ' '; e107::getDB()->gen($sql, false); $info = e107::getDB()->fetch(); //   print_a($info);
        e107::getEvent()->trigger('enquiryNotify', $info); return true; }
else
{
    e107::getMessage()->addError(LAN_PLUGIN_ENQUIRY_FRONT_ERROR_SUBMIT); return false; }
}
else
{
e107::getMessage()->addError(LAN_PLUGIN_ENQUIRY_FRONT_ERROR_CAPTURE); return false; }
...
{
$address = strtolower($address); $coded = ""; $unmixedkey = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.@-"; $inprogresskey = $unmixedkey; $mixedkey = ""; $unshuffled = strlen($unmixedkey);
for ($i = 0; $i <= strlen($unmixedkey); $i++)
{
$ranpos = rand(0, $unshuffled - 1); $nextchar = $inprogresskey{$ranpos}; $mixedkey .= $nextchar; $before = substr($inprogresskey, 0, $ranpos); $after = substr($inprogresskey, $ranpos + 1, $unshuffled -
    ($ranpos + 1)); $inprogresskey = $before . '' . $after; $unshuffled -= 1; }
$cipher = $mixedkey; $shift = strlen($address); $txt = "<script type=\"text/javascript\" language=\"javascript\">\n" . "<!-" . "-\n" .
...
{
    $chr = $address{$j}; $coded .= $address{$j}; }
else
{
    $chr = (strpos($cipher, $address{$j}) + $shift) % strlen($cipher); $coded .= $cipher{$chr}; }
}
...
"/script><noscript>N/A" . "<" . "/noscript>"; return $txt; }
}
