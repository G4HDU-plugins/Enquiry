...
class enquiry_class
{
    ...
    function __construct()
    {
        ...
        e107::lan('enquiry'); // load language file ie. e107_plugins/enquiry/languages/English.php
    ...
    public function runPage()
    {
        ...
        } else
        {
            ...
            {
                // not verified code
                e107::getMessage()->addError(LAN_PLUGIN_ENQUIRY_FRONT_INVALIDCAP);
                $this->action = 'form';
            }
            if (isset($_POST['submit']) && 
                (empty($_POST['enquiry_name']) || 
                empty($_POST['enquiry_address1']) ||
                empty($_POST['enquiry_town']) ||
                empty($_POST['enquiry_postcode']) ||
                empty($_POST['enquiry_email']) 
                )
                )
            {
                e107::getMessage()->addError(LAN_PLUGIN_ENQUIRY_FRONT_MISSFIELDS);
                $this->action = 'form';
            }
            switch ($this->action)
            {
    ...
    function doSave()
    {
        ...
        if (!$this->image || ($this->image && isset($_POST['rand_num']) && $this->verified_code))
        {
            ...
            WHERE enquiry_id = ' . $id . ' ';
                ...
                $info = e107::getDB()->fetch(); //   print_a($info);
